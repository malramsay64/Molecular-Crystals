...
/* Anyu */
void define_anyu(struct shapetype *shape, double cradii[2]) {
    /* cradii[0] means the distance d between circle centers
       cradii[1] means the radius of the second circle (the first was 1.0) */
    int i;
    double phi;
    double m;

    sprintf(shape->name, "%s", "anyu");
    shape->maxr=0.0;
    shape->minr=100.0;

    m=((pow(cradii[1],2)-1+pow(cradii[0],2)))/(2*cradii[0]);
    //m=cradii[1]-(cradii[1]+1-cradii[0])/2

    fprintf(stderr, "anyu params : %f %f %f\n", cradii[0], cradii[1], m);
    //exit(1);

    for (i=0; i<SHAPE_RESOLUTION; i++) {
        phi = 2.0*M_PI*i/SHAPE_RESOLUTION;
        if (cos(phi)>0.0) {
            shape->r[i] = m*cos(phi)+sqrt(fabs(-pow(m,2)*pow(sin(phi),2)+pow(cradii[1],2)));
        } else { 
            shape->r[i] =  -(cradii[0]-m)*cos(phi)+sqrt(-pow((cradii[0]-m),2)*pow(sin(phi),2)+1);
        }
        printf("%f\n", -m*m*sin(phi)*sin(phi)+cradii[1]*cradii[1]);
        printf("%f %f\n", phi, shape->r[i]);
        if (shape->r[i] <= 0.0) {
            fprintf(stderr, "shape has 0 or negative radius at phi=%f %d\n", phi, i);
            exit(1);
        }
...
/* Peter */
void define_peter(struct shapetype *shape, double cradii[2]) {
    /* the second parameter is actually an angle, not a radius */
    int i;
    double phi;
    double theta_1=cradii[1]*M_PI/180.0;
    double theta_2=theta_1/2.0;
    double theta_3=acos((2.0-cradii[0]*cradii[0])/2.0);;

    sprintf(shape->name, "%s", "peter");
    shape->maxr=0.0;
    shape->minr=100.0;

    fprintf(stderr, "peter params : %f %f %f %f %f\n", cradii[0], cradii[1], theta_1, theta_2, theta_3);
    //exit(1);

    for (i=0; i<SHAPE_RESOLUTION; i++) {
        phi = 2.0*M_PI*i/SHAPE_RESOLUTION;   
        if ((phi > theta_2-theta_3) && (phi<theta_2+theta_3)) { 
            shape->r[i] = (cos(phi-theta_2)+sqrt(cos(phi-theta_2)*cos(phi-theta_2)-1.0+cradii[0]*cradii[0]));	  
            printf("part a r=%f\n", shape->r[i]);
        } else if ((phi < 2.0*M_PI-theta_2+theta_3) && (phi>2.0*M_PI-theta_2-theta_3)) { 
            shape->r[i] = (cos(phi-2.0*M_PI+theta_2)+sqrt(cos(phi-2.0*M_PI+theta_2)*cos(phi-2.0*M_PI+theta_2)-1.0+cradii[0]*cradii[0]));	  
            printf("part b r=%f\n", shape->r[i]);
        } else {
            shape->r[i] = 1.0;
            printf("part c r=%f\n", shape->r[i]);
        ...
        if (shape->r[i] <= 0.0) {
            // fprintf(stderr, "ashape has 0 or negative radius at phi=%f %d %f r=%f+%f\n", phi, i, phi-theta_2, cos(phi-theta_2), sqrt(cos(phi-theta_2)*cos(phi-theta_2)-1.0+cradii[0]*cradii[0]));
            //fprintf(stderr, "bshape has 0 or negative radius at phi=%f %d %f r=%f+%f\n", phi, i, phi-2.0*M_PI+theta_2, cos(phi-2.0*M_PI+theta_2), sqrt(cos(phi-2.0*M_PI+theta_2)*cos(phi-2.0*M_PI+theta_2)-1.0+cradii[0]*cradii[0]));
            fprintf(stderr, "shape has 0 or negative radius at phi=%f %d\n", phi, i);
            exit(1);
        }
        if (shape->r[i]>shape->maxr) shape->maxr = shape->r[i]; 
    ...
    shape->rotsym=1;
    shape->mirrors=1; /* may need a second mirror when angle = 180 */
}

/* Ceri */
void define_ceri(struct shapetype *shape, double cradii[2]) {
    int i;
    double phi;
    double theta_1;
    double theta_2;
    ...
    sprintf(shape->name, "%s", "ceri");
    shape->maxr=0.0;
    shape->minr=100.0;

    theta_1 = acos((2.0-cradii[0]*cradii[0])/2.0);
    alpha = acos((2.0-(cradii[1]/cradii[0])*(cradii[1]/cradii[0]))/2.0);
    x2 = 1.0+cradii[0]*cos(M_PI/2.0 + theta_1/2.0 - alpha);
    y2 = cradii[0]*sin(M_PI/2.0 + theta_1/2.0 - alpha);
    theta_2 = atan(y2/x2);
    theta_3 = acos((2.0-cradii[1]*cradii[1])/2.0);

    fprintf(stderr, "ceri params : %f %f %f %f %f %f %f %f\n", cradii[0], cradii[1], theta_1, theta_2, theta_3, alpha, x2, y2);
    //exit(1);

    for (i=0; i<SHAPE_RESOLUTION; i++) {
        phi = 2.0*M_PI*i/SHAPE_RESOLUTION;   
        if ((phi > 2.0*M_PI - theta_1) || (phi<theta_2)) { 
            shape->r[i] = (cos(phi)+sqrt(cos(phi)*cos(phi)-1.0+cradii[0]*cradii[0]));
        } else if (phi<(theta_1+theta_3)) {
            shape->r[i] = (cos(phi-theta_1)+sqrt(cos(phi-theta_1)*cos(phi-theta_1)-1.0+cradii[1]*cradii[1]));	  
        } else {
            shape->r[i] = 1.0;
        }
        if (shape->r[i] <= 0) {
    ...
    shape->rotsym=1;
    if (cradii[1]<0.00000000001) {
        /* it's actually a dumbbell */
        shape->mirrors = 1;
    } else {
        shape->mirrors = 0; /* may not be true ... must adjust if cradii[1]==cradii[0] */
    }
}
...
void define_heart(struct shapetype *shape) {
    int i;
    double phi;
    sprintf(shape->name, "%s", "heart");
    shape->maxr=0.0;
    shape->minr=100.0;
    for (i=0; i<SHAPE_RESOLUTION; i++) {
        phi = 2.0*M_PI*i/SHAPE_RESOLUTION;   
        phi = phi-M_PI/2.0;
        shape->r[i] = 2.0-2.0*sin(phi)+sin(phi)*sqrt(fabs(cos(phi)))/(sin(phi)+1.4);
        if (shape->r[i]>shape->maxr) shape->maxr = shape->r[i]; 
        if (shape->r[i]<shape->minr) shape->minr = shape->r[i]; 
    }
    shape->rotsym = 1;
