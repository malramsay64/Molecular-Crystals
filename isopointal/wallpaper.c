...
double r_at_angle_greater_than(double theta, struct shapetype *shape_a, double d_limit) {
    int theta_int_floor, theta_int_ceil;

    double c, r1, r2, theta_12, theta_star, alpha, beta, r_star;

    theta_int_floor = (int)floor(theta*SHAPE_RESOLUTION/(2.0*M_PI));
    theta_int_ceil  = (int)ceil(theta*SHAPE_RESOLUTION/(2.0*M_PI));  
    if (theta_int_ceil==SHAPE_RESOLUTION) theta_int_ceil = 0; // wrap back to 0

    /* early cut off if d_limit is larger than the radius at either end of the segment */
    if(d_limit > MAX(shape_a->r[theta_int_floor], shape_a->r[theta_int_ceil])) {
        //printf("early\n");
        return 0;
    }

    r1 = shape_a->r[theta_int_floor];
    r2 = shape_a->r[theta_int_ceil];
    theta_12 = 2.0*M_PI/SHAPE_RESOLUTION;

    c = sqrt(r1*r1+r2*r2-2*r1*r2*cos(theta_12));

    //alpha = asin(r2*sin(theta_12)/c); asin gives an ambiguous result... always acute?  no
    alpha = acos((r1*r1+c*c-r2*r2)/(2.0*r1*c));
    theta_star = theta - theta_int_floor * 2.0*M_PI/SHAPE_RESOLUTION;
    beta = M_PI - alpha - theta_star;
    r_star = sin(alpha)*r1/sin(beta);

    //printf("%f %f %f (floor %f %d) c=%f (angles theta12=%f thetastar=%f alpha=%f beta=%f)\n", r1, r2, r_star, floor(theta*SHAPE_RESOLUTION/(2.0*M_PI)), theta_int_floor, c, theta_12, theta_star, alpha, beta);

    if (d_limit > r_star) return 0; 
    else return 1;
}

bool malcolm_clash(struct shapetype *shape_a, struct shapetype *shape_b, double coords_a[DIM], double coords_b[DIM], double angle_a, double angle_b, int rotation_a, int rotation_b, bool flip_a, bool flip_b, int tests){

    double delta[DIM];
    double center_dist_sq = 0;
    int i;
    for (i = 0; i < DIM; i++){
        delta[i] = coords_a[i] - coords_b[i];
        center_dist_sq += delta[i]*delta[i];
    }
    double rel_angle = atan2(delta[0], delta[1]);
    int start_a = rel_angle/SHAPE_RESOLUTION;
    int start_b = (rel_angle+M_PI)/SHAPE_RESOLUTION;
    start_a += (1-2*flip_a)*rotation_a;
    start_b += (1-2*flip_b)*rotation_b;
    double dtheta = 2*M_PI/SHAPE_RESOLUTION;
    double x_a, x_b, t_a, t_b;
    for (i = 0; i < SHAPE_RESOLUTION; i++){
        x_a = shape_a->r[(start_a+i)%SHAPE_RESOLUTION];
        x_b = shape_b->r[(start_b+i)%SHAPE_RESOLUTION];
        t_a = dtheta*(start_a+i);
        t_b = dtheta*(start_b+i);
        if (pow(x_a*cos(t_a),2) + pow(x_b*cos(t_b),2) > center_dist_sq){
            return 1;
        }
    }
    return 0;
...
bool indirect_clash(struct shapetype *shape_a, struct shapetype *shape_b, double coords_a[DIM], double coords_b[DIM], double angle_a, double angle_b, bool flip_a, bool flip_b,  double central_distsq, int tests) {
    ...
    double xa_tab[SHAPE_RESOLUTION+1]; //+1 is for dummy radius of 0 (at center of shape)
    double ya_tab[SHAPE_RESOLUTION+1];
    double xb_tab[SHAPE_RESOLUTION+1];
    double yb_tab[SHAPE_RESOLUTION+1];
    double centraldist;
    double distsq_ia_b, dist_ia_b;
    double distsq_ib_a, dist_ib_a;
    double point_ia_angle_at_b;
    double point_ib_angle_at_a;
    int ia;
    int ib;

    /* this is the most expensive routine of all - try to optimize */

    centraldist = sqrt(central_distsq);

    //We want to check all the way around the edge of each shape, to make sure that by chance the two shapes aren't positioned on top of each other.
    for (ib = 0; ib<=SHAPE_RESOLUTION; ib++) {
        ...
        // MAY WANT TO PUT A CONTINUE CONDITION IN HERE IF THIS DISTANCE IS WAY TOO BIG TO CAUSE CLASH
        dist_ib_a = sqrt(distsq_ib_a);

        // now compute what angle that would be at from the center of particle a
        if (xb_tab[ib]>0.0) {
            point_ib_angle_at_a = atan(yb_tab[ib]/xb_tab[ib]);
        } else {
            point_ib_angle_at_a = atan(yb_tab[ib]/xb_tab[ib])+M_PI;
        }
        //  (relative to it's internal reference frame)
        if (flip_a) {
            point_ib_angle_at_a -= angle_a;
        } else {
            point_ib_angle_at_a += angle_a;
        }
        //point_ib_angle_at_a *= -1.0;
        while (point_ib_angle_at_a < 0.0) point_ib_angle_at_a += 2.0*M_PI;
        while (point_ib_angle_at_a > 2.0*M_PI) point_ib_angle_at_a -= 2.0*M_PI;


        //printf("\tib=%d\ttheta=%f, dist_ib_a=%f ", ib, point_ib_angle_at_a*180/M_PI, dist_ib_a);

        if ( r_at_angle_greater_than(point_ib_angle_at_a, shape_a, dist_ib_a) ) return 1;


    }


    for (ia = 0; ia<SHAPE_RESOLUTION; ia++) {
        if (flip_a == 0) {
            xa_tab[ia] = -centraldist + shape_a->r[ia] * cos(-angle_a + ia*2.0*M_PI/SHAPE_RESOLUTION);
            ya_tab[ia] =                shape_a->r[ia] * sin(-angle_a + ia*2.0*M_PI/SHAPE_RESOLUTION);
        } else {
            xa_tab[ia] = -centraldist + shape_a->r[ia] * cos(angle_a - ia*2.0*M_PI/SHAPE_RESOLUTION);
            ya_tab[ia] =                shape_a->r[ia] * sin(angle_a - ia*2.0*M_PI/SHAPE_RESOLUTION);
        }

        // compute what distance ia is from the center of particle a
        distsq_ia_b = (ya_tab[ia])*(ya_tab[ia]) + (xa_tab[ia])*(xa_tab[ia]);
        // MAY WANT TO PUT A CONTINUE CONDITION IN HERE IF THIS DISTANCE IS WAY TOO BIG TO CAUSE CLASH
        dist_ia_b = sqrt(distsq_ia_b);

        // now compute what angle that would be at from the center of particle b
        if (xa_tab[ia]>0.0) {
            point_ia_angle_at_b = atan(ya_tab[ia]/xa_tab[ia]);
        } else {
            point_ia_angle_at_b = atan(ya_tab[ia]/xa_tab[ia])+M_PI;
        }

        point_ia_angle_at_b -= M_PI; /* because b is on the right */

        //  (relative to it's internal reference frame)
        if (flip_b) {
            point_ia_angle_at_b = angle_b - point_ia_angle_at_b ;
        } else {
            point_ia_angle_at_b = +angle_b + point_ia_angle_at_b;
        }
        //point_ia_angle_at_b *= -1.0;
        while (point_ia_angle_at_b < 0.0) point_ia_angle_at_b += 2.0*M_PI;
        while (point_ia_angle_at_b > 2.0*M_PI) point_ia_angle_at_b -= 2.0*M_PI;

        //printf("\tib=%d\ttheta=%f, dist_ib_a=%f ", ia, point_ia_angle_at_b*180/M_PI, dist_ia_b);

        if ( r_at_angle_greater_than(point_ia_angle_at_b, shape_b, dist_ia_b) ) return 1;
...
bool pair_clash(struct shapetype *shape_a, struct shapetype *shape_b, double coords_a[DIM], double coords_b[DIM], double orient_a, double orient_b, int rotation_a, int rotation_b, bool flip_a, bool flip_b, int tests) {
    ...
    //printf("inclines %f %f\n", a_to_b_incline*180/M_PI, b_to_a_incline*180/M_PI);
...
bool clash_polygon(struct shapetype *shape_a, struct shapetype *shape_b, struct wallpaper_group_type group, double *cellsides[DIM], double *cellangles[DIM-1],  int occupiedsites[MAXNUMOCCSITES], double *sitevariables[MAXNUMOCCSITES][3], int rep_a[2], int rep_b[2], bool flips[numoccsites], int tests) {
    ...
    /* now expand to 7x7 instead of 3x3*/
    for (cellimgx=-3; cellimgx<=3; cellimgx++) { 
        ...
        for (cellimgy=-3; cellimgy<=3; cellimgy++) { 
            ...
            //printf("img_fcoords_a %f %f img_fcoords_b %f %f ", fcoords_a[0], fcoords_a[1], img_fcoords_b[0], img_fcoords_b[1] );
    ...
    *clash_rejection=0;
    ...
    int initialize_structure_in_group(struct shapetype *shape, struct wallpaper_group_type group, int occupiedsites[MAXNUMOCCSITES], double basis[MAXVAR], int basis_fixed[MAXVAR], double basis_range[MAXVAR][2], double *cellsides[DIM], double *cellangles[DIM-1],  double *sitevariables[MAXNUMOCCSITES][3], bool flips[MAXNUMOCCSITES], int checkfile, int *isfile) {
        ...
        if (group.a_b_equal) {
            ...
            countvar+=1;
            basis[1] = 4.0*shape->maxr*sqrt(countreplicas);     /* NEW: 2012-12-10: starting cell edge length, start large to avoid overlaps, and there will be an early scaling down to remove free space and get a reasonable cell size. determine initial size based on number of replicas per cell */
            basis_range[1][0] = 0.1;
            ...
            countvar+=2;
            basis[1] = 4.0*shape->maxr*sqrt(countreplicas);     /* NEW 2012-12-10: starting cell edge length */
            basis[2] = 4.0*shape->maxr*sqrt(countreplicas);     /* NEW 2012-12-10: starting cell edge length */
            basis_range[1][0] = 0.1;
            basis_range[1][1] = basis[1];
            basis_range[2][0] = 0.1;
        ...
        sprintf(filename, "solution_%s_%s_%s_%.3f.svg", shape->name, group.label, oslist, shape_var);
    ...
    void uniform_best_packing_in_isopointal_group(struct shapetype *shape, struct wallpaper_group_type group, int occupiedsites[MAXNUMOCCSITES]) {
        double phi;
        double kT_start = 0.6; //Daniel used 0.1
        double kT_finish = 0.0001; //Daniel used 0.0005
        double kT_ratio;
        double kT;
        ...
        for (cycle=0; cycle<CYCLES; cycle++) {
            ...
            while (tests<MAXSTEPS) {
                ...
                fprintf(fpout,"%8s\t%s\t%d\t%.6f\t%.7f\t%.3f\t%s\t%d\t%.5f\t%d\t%.3f\t%d\t%.4f\t%.4f\t%.4f\t%.6f\t%.6f\t%.6f\t%.6f\n", group.label, oslist, cycle+1, max_phi, area(shape), (100.0*rejections)/tests, shape->name, MAXSTEPS, max_step_size, SHAPE_RESOLUTION, shape_var, numoccsites, shape->minr, shape->maxr, shape->minr/shape->maxr, max_phi_in_cycle, *cellsides[0], *cellsides[1], *cellangles[0]*180.0/M_PI);

                if (!SUPPRESS_ISOPOINTAL_OUTPUT) {
            ...
            /*
               printf("bag contents: ");
               for (i=0; i<curr_bag; i++) {
               printf("%d ", combination_bag[i]);
               }
               printf("\n");
               */
            ...
            for (i=0; i<numcombinations; i++) {
                ...
                char format[] = "output_%s_%s_%c_%d.dat"; //DAN this paragraph
                char filename[sizeof format+100];
                sprintf(filename,format,shape->name,group.label,group.wyckoffs[occupiedsites[i][0]].letter,numoccsites);
        ...
        int main (int argc, char *argv[]) {
            ...
            struct shapetype anyu;
            struct shapetype peter;
            struct shapetype ceri;
            ...
            if (SHAPE_RESOLUTION%12 !=0) {
                fprintf(stderr, "shape resolution must be divisible by 2, 3, 4, and 6 to ensure that all Wyckoff sites can be correctly rotated - i.e. divisible by 12\n");
                exit(1);
            }
            ...
            } else if ((strcmp(argv[2], "anyu")==0)||(strcmp(argv[2], "ceri")==0)||(strcmp(argv[2], "peter")==0)) {
                if (argc!=6) {
                    fprintf(stderr, "wrong number of arguments. wallpaper.exe num_occ_sites shape_name wallpaper_group_num/all/best5/best8 coefficients(r2, r3)\n");
                    exit(1);
                }
            } else {
                if (argc!=4) {
            ...
            sprintf(datfilename, "%s_%d_%.3f.dat", argv[2], SHAPE_RESOLUTION, shape_var);
            ...
            char format[] = "output_%s_%.3f_%d.dat";  //DAN this paragraph
            char filename[sizeof format+100];
            sprintf(filename,format,chosen_shape->name,shape_var,numoccsites);
