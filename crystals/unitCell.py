...
class cell:
    ...
    def addParticle(self,a,b,theta):
        m = copy(self.mol)
        m.setAngle(theta)
        m.translate(a*self.a,b*self.b)
        self.mols.append(m)

    def addMol(self, a, b, theta):
        self.addParticle(a,b,theta)
...
class p2gg(cell):
    ...
    def __init__(self, a, b, theta, x, y, phi, mol):
        ...
        x = min(x, 1-x)
        self.addMol(x,y,phi)
        self.addMol((1-x)%1,(1-y)%1,phi+180)
        self.addMol(1-((0.5+x)%1), (0.5+y)%1, phi)
        self.addMol(1-((0.5+(1-x))%1), (0.5+(1-y))%1, 180-phi)
