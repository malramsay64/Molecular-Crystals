...
set output plot_dir.shape(word(files,1)).'-msd'.ext
...
set output plot_dir.shape(word(files,1))."-r1".ext
unset logscale y
set format y "%g"
set ylabel "R1"
set yrange [-0.1:1]
...
set output plot_dir.shape(word(files,1))."-r2".ext
set ylabel "R2"

plot for [i=1:words(files)] word(files, i)."/rotation.csv" using 1:3 with linespoints linestyle 5\
     linecolor i title temp(word(files,i)), 0 linecolor 'black', 1/e linecolor 'black'

set output plot_dir.shape(word(files,1))."-struct".ext
